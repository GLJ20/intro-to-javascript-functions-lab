...
const greetUser = (name, timeof) =>{
    return `Good ${timeof}, ${name}!`
}
...
const calculateTip = (bill, tip) => {
    const tipprcnt = tip/100;
    const amountoftip = bill * tipprcnt;
    return amountoftip;
}
...
const createUsername = (fn, ln) =>{
    const f3ltrfn = fn.slice(0, 3);
    const f3ltrln = ln.slice(0,3);
    const totalchar = fn.length + ln.length;
    return `${f3ltrfn}${f3ltrln}${totalchar}`;
}
